...
describe("Podcast playback progress", () => {
  it("isolates progress by user, episode, and resolved locale", () => {
    expect(podcastProgressKey("user-1", "episode-1", "zh-TW")).toBe(
      "daily-insights:podcast-progress:v1:user-1:episode-1:zh-TW"
    )
    expect(podcastProgressKey("user-1", "episode-1", "en")).not.toBe(
      podcastProgressKey("user-1", "episode-1", "zh-TW")
    )
  })
