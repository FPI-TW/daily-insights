...
| 群組     | 標的                                                            | 用途                                  |
| -------- | --------------------------------------------------------------- | ------------------------------------- |
| 大盤代理 | `SPY`、`QQQ`、`DIA`、`IWM`                                      | 四大指數的 ETF 代理，避開指數授權問題 |
| 波動率   | `VIX`（或 `VIXY`）                                              | 風險偏好                              |
| 權值股   | `AAPL`、`MSFT`、`NVDA`、`GOOGL`、`AMZN`、`META`、`AVGO`、`TSLA` | 客戶實際關心的標的                    |

用 ETF 代理而非指數本身，順帶避開先前盤點提到的 S&P / Nasdaq 指數授權未確認問題。

### 顯示方式

分成兩個 block 比擠成一張表好讀：

- `us.index_proxies` —— `metric` block，四個大盤代理 + VIX
- `us.mega_caps` —— `table` block，權值股，欄位為 標的 / 價格 / 漲跌幅，
  **依漲跌幅排序**（籃子固定，排序不影響選樣，就沒有低價股問題）

### 成本

`TwelveDataAdapter.get_quote` 是**單一 symbol** 的（`adapter.py:77`），
13 檔就是 13 次呼叫。免費層每分鐘 8 credits，會超過。三個選項：

- 分批呼叫並在批次間 sleep（最簡單，晨報是排程任務不在乎多花 60 秒）
- 縮減籃子到 8 檔以內
- 確認 Twelve Data 的 `/quote` 是否支援逗號分隔的多 symbol 批次查詢
  （**本規格未查證**，若支援則一次呼叫解決，優先確認這條）
...
| **每個 block 只能引用 1 個 dataset**             | 拆成兩個 block 就要有**兩個** dataset key                                                               |
| **每個 dataset 只能被 1 個市場引用**             | 兩個新 dataset 都掛 `us_equity`，符合                                                                   |
| `symbol_units` 必須涵蓋每個 symbol               | 現行 `us.market_movers` 是 `symbols=()`、`symbol_units={}`（空的所以通過）。新 dataset 要逐一列出 13 檔 |
| **每個 unit 必須是三碼大寫**（`^[A-Z]{3}$`）     | 全部填 `"USD"`                                                                                          |
| 每個 block 必須有三語 labels                     | 兩個新 block 各要 zh-hant / zh-hans / en                                                                |
...
1. 確認 `/quote` 是否支援批次 symbol（決定呼叫次數與是否需要分批 sleep）
2. 確認美元計價的銅代碼是否存在
3. `launch_manifest.py`：新增 `us.index_proxy_quotes` 與 `us.mega_cap_quotes` 兩個
   DatasetManifest（`endpoint="/quote"`，逐一列出 symbols 與 `symbol_units`），
   新增對應的兩個 BlockManifest（含三語 labels），移除 `us.market_movers`
   的 block 與 dataset
4. `launch_manifest.py`：`version` bump
5. `morning_report.py`：新增這兩個 dataset 的 build 分支，移除 `us.market_movers` 分支；
